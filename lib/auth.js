...
const getAuth = async () => {
  ...
  const db = mongoose.connection.db || (mongoose.connection.client ? mongoose.connection.client.db('StudyNook') : null);
  if (!db) {
    throw new Error("MongoDB Database instance is not available.");
  }

  const serverBaseUrl = process.env.BETTER_AUTH_URL || (process.env.NODE_ENV === 'production' || process.env.VERCEL ? 'https://study-nook-server-silk.vercel.app' : 'http://localhost:5000');
